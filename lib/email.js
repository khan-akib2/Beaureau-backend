...
export const sendRoleChangeEmail = async (to, name, newRole) => {
  ...
  const title = isAdmin ? "Admin Access Granted" : "Admin Access Removed";
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || '"BureauAI Admin" <a66071001@smtp-brevo.com>',
    to,
    subject: `BureauAI: ${title}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
        <h2 style="color: #1a56db; margin: 0 0 18px; font-size: 22px;">${title}</h2>
        ...
        <div style="margin: 20px 0; padding: 14px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px;">
          <p style="margin: 0; font-size: 13px; color: #334155;">Current account role: <strong>${isAdmin ? "Administrator" : "User"}</strong></p>
        </div>
        <p style="font-size: 12px; color: #64748b; border-top: 1px solid #e2e8f0; padding-top: 14px;">
          This email was sent because the primary BureauAI administrator changed your account access.
        </p>
      </div>
    `,
  };
  ...
  try {
    const info = await transporter.sendMail(mailOptions);
    console.log("Role change email sent successfully:", info.messageId, "to:", to);
    return { success: true, info };
  } catch (error) {
