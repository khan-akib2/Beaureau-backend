...
export const seedRealDb = async () => {
  try {
    ...
    if (!demoUser) {
      ...
      if (demoUser.emailNotifs === undefined) demoUser.emailNotifs = true;
      if (demoUser.smsNotifs === undefined) demoUser.smsNotifs = true;
      if (demoUser.appNotifs === undefined) demoUser.appNotifs = true;
      if (demoUser.statusUpdates === undefined) demoUser.statusUpdates = true;
      if (demoUser.isAadhaarLinked === undefined) demoUser.isAadhaarLinked = false;
      if (demoUser.isDigiLockerLinked === undefined) demoUser.isDigiLockerLinked = false;
      await demoUser.save();
    ...
    if (!adminUser) {
      ...
      if (adminUser.emailNotifs === undefined) adminUser.emailNotifs = true;
      if (adminUser.smsNotifs === undefined) adminUser.smsNotifs = true;
      if (adminUser.appNotifs === undefined) adminUser.appNotifs = true;
      if (adminUser.statusUpdates === undefined) adminUser.statusUpdates = true;
      if (adminUser.isAadhaarLinked === undefined) adminUser.isAadhaarLinked = false;
      if (adminUser.isDigiLockerLinked === undefined) adminUser.isDigiLockerLinked = false;
...
async function dbConnect() {
  if (!MONGODB_URI) {
    console.warn("⚠️ MONGODB_URI environment variable is missing! Falling back to mock-db.json.");
    seedMockDb();
    return { isMock: true };
  }
  ...
  if (!cached.promise) {
    cached.promise = mongoose
      ...
      .catch((err) => {
        console.error("❌ MongoDB connection failed! Falling back to mock-db.json.", err.message);
        seedMockDb();
        return { isMock: true };
      });
  }
