...
async function getRefusal(query) {
  try {
    const refusalPrompt = `The user asked: "${query}".
Write a polite, extremely short 1-sentence refusal (under 25 words) in the user's query language (e.g. Hinglish, Hindi, Bengali, Marathi, Urdu, English) stating that you are an AI assistant integrated specifically into the Bureau portal and cannot help with unrelated queries.
...
export async function chatWithGemini(messages, userLanguage = "en") {
  try {
    ...
    if (latestQuery && !latestUserMsg.fileUrl) {
      ...
      if (!related) {
        return await getRefusal(latestQuery);
      }
    }
    ...
    const completion = await groq.chat.completions.create({
      model: hasImage ? "meta-llama/llama-4-scout-17b-16e-instruct" : MODEL,
