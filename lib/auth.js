...
const ACTUAL_JWT_SECRET = JWT_SECRET || "bureauai_super_secret_jwt_key_987654321";
...
export function requireAuth(req, res, next) {
  ...
  // Reject mock session tokens ONLY in production to allow local testing
  const isRealObjectId = mongoose.Types.ObjectId.isValid(decoded.id);
  const isMockId = typeof decoded.id === "string" && decoded.id.startsWith("user_");

  if (!isRealObjectId && !isMockId) {
    return res.status(401).json({ error: "Invalid session token." });
  }

  if (process.env.NODE_ENV === "production" && isMockId) {
    return res.status(401).json({ error: "Mock accounts are disabled in production. Please sign in with a real account." });
  }
